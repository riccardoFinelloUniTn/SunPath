...
impl<T: Copy> ArenaKeyMappedBuffer<T> {
    ...
    pub fn insert(&mut self, id: u64, data: &T) -> SrResult<(usize, vk::BufferCopy)> {
        let slot = if let Some(&existing_slot) = self.id_map.get(&id) {
            ...
            let slot = self.ring.allocate_slot()?;
...
pub struct ArenaGpuKeyMappepBuffer<T: Copy> {
    ring: ArenaRingCore<T>,
    id_map: HashMap<u64, usize>,
...
impl<T: Copy> ArenaGpuKeyMappepBuffer<T> {
    ...
    pub fn remove(&mut self, id: u64) -> Option<vk::BufferCopy> {
        let slot = self.id_map.remove(&id);
        if let Some(s) = slot {
            self.ring.free_slot(s);
            self.write_mapping_slot(s, Self::EMPTY_SLOT).ok()
        } else {
            None
        }
    }
